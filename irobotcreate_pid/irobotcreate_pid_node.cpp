...
int main(int argc, char **argv)
{
  ...
  while (ros::ok())
  {
    ...
    float abs_diff = sqrt(pow(abs_diff_x,2) + pow(abs_diff_y,2)); 
	...
	if(ang_diff > M_PI && ang_diff < 2*M_PI)
	{
	  ...
	  float derivative = yaw - prior_ang_pos;
	...
	else
	{
	  ...
	  float derivative = prior_ang_neg - yaw;
	...
	float derivative = pos - prior_lin;
